...
export function isValidPublicKey(publicKey) {
  try {
    const keyBuffer = utils.PublicKey.from(publicKey).data;
...
export async function getStakedValidators(wallet) {
  ...
  let mp = await getMyPools(wallet);
  mp = { ...mp.myPools, ...mp.addedPools };
  for (const pool in mp) {
    if (!pools.find((obj) => obj.account_id === pool))
  ...
  for (const pool of pools) {
    ...
    try {
      ...
      if (
        Decimal(totalBalance).div(yoctoNEAR).toFixed(2) > 0 ||
        Decimal(stakedBalance).div(yoctoNEAR).toFixed(2) > 0 ||
        Decimal(unstakedBalance).div(yoctoNEAR).toFixed(2) > 0
      ) {
...
export async function getMyPools(wallet) {
  ...
  let addedPools = {};
  for (const validator of validators.addedPools) {
    try {
      const fee = await wallet.viewMethod({
        contractId: validator.pool_id,
        method: 'get_reward_fee_fraction',
  ...
  return { myPools, addedPools };
}

export async function getPools(wallet) {
  let validators;
  const requestOptions = {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
