...
const DataForm = (props) => {
  ...
  const handleChangeFee = (e) => {
    if (e.target.value >= 0 && e.target.value <= 100) {
      setPercentageFee(parseInt(e.target.value));
      setPercentageFeeErrorText('');
    }
  };

  const handlePublicKeyChange = async (e) => {
    setPublicKey(e.target.value);
    setPublicKeyErrorText('');
    setKeyPair(null);
    if (await isAvailableAccountID())
  ...
  const handleGenerateKey = async () => {
    if (!(await isAvailableAccountID())) return false;
    if (poolName) {
  ...
  const isAvailableAccountID = async () => {
    if (!/^[a-z0-9_-]{2,}$/.test(poolName)) {
      setPoolNameErrorText(
        'Pool name must contain only lowercase letters (a-z), digits (0-9), and the characters _ and -, and it must contain at least 2 characters'
      );
      return false;
    }
    if (
  ...
  const checkForm = async () => {
    ...
    if (!publicKey || !isValidPublicKey(publicKey)) {
      setPublicKeyErrorText('Please enter valid Public key');
      return false;
    }
  ...
  const handleContractPoolChange = (e) => {
    setContractPool(e.target.value);
  };

  const handleOwnerAccount = (e) => {
            ...
            <Grid container spacing={{ xs: '12px', sm: '24px', md: '48px' }}>
              ...
              <Grid item xs={6}>
                <TextField
                  ...
                  id="contractPool"
                  value={contractPool}
                  onChange={handleContractPoolChange}
                  SelectProps={{
...
export default function CreateStakingPool({ wallet }) {
  return (
    <Box sx={{ width: '100%' }}>
