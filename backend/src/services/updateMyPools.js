...
const getMyPools = async (network, account_id) => {
  try {
    const pgClient = new pg.Client({
      connectionString:
        network === 'mainnet'
          ? process.env.MAINNET_POSTGRESQL_CONNECTION_STRING
          : process.env.TESTNET_POSTGRESQL_CONNECTION_STRING,
...
const getNewPool = async (network, account_id, txHash) => {
  try {
    const NEAR_RPC_URL =
      network === 'mainnet'
    ...
    const tx = await provider.txStatus(txHash, account_id);
    if (tx.status.hasOwnProperty('SuccessValue')) {
      const method_name = tx.transaction.actions[0].FunctionCall.method_name;
      if (method_name === 'create_staking_pool') {
        ...
        return {
          pool_id: args.staking_pool_id + '.' + tx.transaction.receiver_id,
...
export const updateMyPools = async (req, res) => {
  try {
    const newPool = await getNewPool(
      req.body.network,
      req.body.account_id,
      req.body.txHash
    );
    if (newPool) {
      ...
      Pools.findOneAndUpdate(
        { account_id: newPool.signer_id, pool_id: newPool.pool_id },
        {
          account_id: newPool.signer_id,
      ...
      res.status(500).send({ error: 'pool not found' });
    }
  } catch (e) {
    console.log(e);
    res.status(500).send({ error: 'Please try again' });
  ...
  try {
    ...
    for (const account of accounts) {
      ...
      for (const pool of myPools) {
        Pools.findOneAndUpdate(
          { account_id: account.account_id, pool_id: pool.pool_id },
          {
